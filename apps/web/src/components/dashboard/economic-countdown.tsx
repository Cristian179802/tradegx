...
export function EconomicCountdown({ className }: { className?: string }) {
  const t = useTranslations("economicCountdown");
  ...
  React.useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setRemaining(t("now")); return; }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-zinc-500 mt-0.5">{t("nextImpactNews")}</p>
      </div>
      <div className="text-right shrink-0">
        <p className={cn("text-lg font-black num leading-none", soon ? "text-amber-300" : "text-zinc-200")}>{remaining}</p>
        <p className="text-[10px] text-zinc-600 mt-0.5">{t("remaining")}</p>
      </div>
    </Link>
  );
}
